...
static sgx_status_t SGX_CDECL sgx_get_table(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_get_table_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_get_table_t* ms = SGX_CAST(ms_get_table_t*, pms);
	...
	get_table(_in_buf, _tmp_len, ms->ms_idx);
	if (_in_buf) {
...
SGX_EXTERNC const struct {
	...
	1,
	{
		{(void*)(uintptr_t)sgx_get_table, 0, 0},
	}
};
